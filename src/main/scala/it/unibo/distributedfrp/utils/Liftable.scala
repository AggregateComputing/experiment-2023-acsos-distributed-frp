...
object Liftable:
  extension[A, F[_] : Liftable] (a: F[A])
    def map[B](f: A => B): F[B] = summon[Liftable[F]].lift(a)(f)

  extension[A, F1[_] : Liftable, F2[_] : Liftable] (a: F1[F2[A]])
    def mapTwice[B](f: A => B): F1[F2[B]] = a.map(_.map(f))
