...
trait ScafiFieldUtils {
  ...
  trait FieldOps {
    ...
    def minHoodSelector[T, V](
        toMinimize: => T
    )(data: => V)(implicit ord1: Builtins.Bounded[T], ord2: Builtins.Bounded[ID]): M[V] = wrap[V] {
      foldhoodTemplate[(T, ID, Option[V])]((ord1.top, ord2.top, None))((x, y) =>
        if (
          x._3.isDefined && (ord1
            .compare(x._1, y._1) < 0 || ord1.compare(x._1, y._1) == 0 && ord2.compare(x._2, y._2) <= 0)
        ) {
          x
        } else y
    ...
    def minHoodLoc[T: Builtins.Bounded](default: T)(expr: => T): T =
      foldhoodTemplate[T](default)(implicitly[Builtins.Bounded[T]].min)(expr)
  }
